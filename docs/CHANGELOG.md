...
- `iss-service.js`: ISSの肉眼観測可能判定を改善しました。
  ...
  - これにより、太陽光が当たっていても空が完全に暗い深夜帯など、実際には観測が困難な時間を除外し、より正確な観測ガイドを提供できるようになりました。

## [3.3.3] - 2026-01-19

### Fixed
- `iss-service.js`: `satellite.sunPos` が未定義のためにISS情報の計算が失敗する問題を修正しました。
  - `satellite.js` v5.0.0 の仕様変更により `sunPos` が直接呼び出せなくなったため、代わりに `Astronomy.js` の `GeoVector` を使用して太陽の位置を計算するように変更しました。
