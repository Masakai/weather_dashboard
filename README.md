...
- **🍔 ハンバーガーメニュー**: 各種設定や各情報パネルへのクイックアクセスが可能
...
- **📱 レスポンシブ & 高解像度対応**: スマートフォン（iPhone Pro Max等）からPCまで、あらゆるデバイスで高い視認性を確保
...
```bash
# リポジトリをクローン
git clone https://github.com/m-sakaimono/weather_dashboard.git
...
- 7日間の天気・気温・降水確率に加え、各日の**夜間の平均視認性スコア**を算出。
- スコアに応じて背景色を色分け表示し、観測に適した日を一目で判別可能。
  - 🟢 80点以上: 絶好
  - 🔵 60点以上: 良好
  - 🟠 40点以上: 注意
  - 🔴 20点以上: 不適
  - 🌑 20点未満: 絶望的
- 日付をクリックすることで、その日の詳細予報へ瞬時に切り替え可能。

### 9. ハンバーガーメニュー & クイックナビゲーション 🍔

- 右上のメニューアイコンから主要機能に素早くアクセス。
- **機能一覧**:
  - 場所の変更（地図表示）
  - 現在地の取得
  - ナイトビジョンモードの切り替え
  - お気に入り地点の管理
- **クイックリンク**:
  - 各種情報パネル（スコア、詳細、計画、イベント、チャート、週間予報）へ即座にスクロール。

### 10. 天体イベント情報 🌟

#### 10-1. ISS（国際宇宙ステーション）追跡 🛰️
...
- **星座図表示機能（地平座標系）** 🌠
  ...
  - **リアルタイム軌跡表示**: リアルタイムモード中、ISSの過去10分間の移動経路を青い線で描画
  - 選択したパスの予測軌道を青い点線で表示（5分刻みマーカー付き）
...
1. **メニューの活用** 🍔
   - 右上の三本線アイコン（ハンバーガーメニュー）をクリックすると、場所の変更、現在地の取得、ナイトビジョンモードの切り替えが可能です。
   - メニュー内のクイックリンクから、各情報セクションに即座に移動できます。

2. **位置情報の設定** 🌍
   - 初回アクセス時に現在地の取得を許可するか、メニューの「現在地を取得」をクリックします。
   - 地図から場所を選びたい場合は、メニューの「場所を変更」またはヘッダーの「地図で場所を変更」をクリックします。

3. **日時の指定** ⏰
   - コントロールバーから任意の日時を選択すると、その時点の予測データに切り替わります。
   - 「現在に戻す」ボタンでいつでも現在の時刻に戻れます。

4. **スコアの確認** ⭐
   - 星空視認性スコア（0-100点）で、今夜の観測適性を判断します。
   - 各要素の詳細（雲量、月齢、湿度など）は気象詳細パネルやレーダーチャートで確認できます。

5. **お気に入り地点の登録** 📍
   - 「お気に入りに追加」ボタン（またはメニュー内のお気に入り管理）から、よく行く観測地を保存できます。
...
- 🛠️ キャッシュバスティング（バージョンクエリ）の整合性調査と全ファイルの一斉更新
- 📦 全モジュールのインポートにおけるバージョンパラメータを 3.3.6 に統一

### v3.3.5 (2026-01-19)
**パッチアップデート**

- 🐛 iss-service.js: 昼間（14:00頃など）にISSが肉眼観測可能と誤表示される問題を修正
- 🔧 SearchRiseSet を使用した日没・日の出判定ロジックを改善

### v3.3.4 (2026-01-19)
**改善アップデート**

- 🛰️ iss-service.js: ISSの肉眼観測可能判定を改善（日没後・日の出前2時間以内に制限）

### v3.3.3 (2026-01-19)
**パッチアップデート**

- 🐛 iss-service.js: satellite.js v5.0.0 仕様変更に伴う太陽位置計算の不具合を修正

### v3.3.2 (2026-01-19)
**マイナーアップデート**

- 🍔 ハンバーガーメニューに各情報パネル（スコア、気象詳細、観測計画、イベント、チャート、週間予報）へのクイックリンクを追加
- ⚡ ページ内ナビゲーションの利便性を大幅に向上

### v3.3.1 (2026-01-19)
**マイナーアップデート**

- 🍔 ハンバーガーメニューを新設。右上のアイコンから主要機能にアクセス可能
- 🛠️ 「場所を変更」「現在地を取得」「ナイトビジョン」などの機能をメニューに集約
- 📱 モバイル端末での操作性を向上

### v3.3.0 (2026-01-19)
**マイナーアップデート**

- 📱 高解像度・大画面デバイス（iPhone Pro Max等）での視認性向上のため、UI全体のデザインを最適化
- 🔍 ヘッダー、コントロールバー、各種パネルのフォントサイズと要素サイズを拡大
- ✨ アイコンサイズの調整とレイアウトの微調整により、情報の読み取りやすさを向上

### v3.2.3 (2026-01-18)
**パッチアップデート**

- 📊 週間天気予報パネルの各日の背景を視認スコアに基づいた透過色で表示
...
- 🛰️ ISS星座図のリアルタイム表示で軌跡を青い線で表示する機能を追加
- 📈 リアルタイムモードでISSの過去の位置を記録し、青い破線で軌跡を描画
- ⏱️ 軌跡データは過去10分間を保持し、古いデータは自動的に削除

### v3.2.1 (2026-01-18)
**パッチアップデート**

- 🌞 日食イベント表示時の観測可能性判定を修正
- 🌙 日食の時刻に現在地で太陽が地平線下にある場合（夜間）は、イベントリストに表示しないように改善
- ✅ 太陽の高度チェックと現地での日食観測可能性を確認し、情報の信頼性を向上

### v3.2.0 (2026-01-18)
**マイナーアップデート**

- 🌕 天文イベント（月食・日食）の詳細情報表示機能を追加
- 📊 各イベントをクリックすることで詳細モーダルを表示
- ⏰ 月食の場合: 部分食開始、皆既開始、食の最大、皆既終了、部分食終了の時刻と継続時間を表示
- 🌍 日食の場合: ユーザーの現在地での観測可能性を自動判定し、食の経過と食分を表示
- 💡 観測アドバイス（安全な観測方法、撮影設定など）を表示

### v3.1.9 (2026-01-18)
**パッチアップデート**

- 📝 CLAUDE.mdに「コミット前の必須チェックリスト」セクションを追加
- ✅ バージョンアップの判断基準と実行手順を明確化
- 📋 作業漏れを防止し、キャッシュ問題やバージョン不整合を防ぐための重要なガイドラインを確立

### v3.1.8 (2026-01-17)
**パッチアップデート**

- 🔧 月食・日食予測のAstroTimeオブジェクト処理を修正
- 🌕 `peak.date`プロパティを使用してDate型に正しく変換
- 🔍 検索範囲の判定ロジックを修正し、すべての月食・日食が正しく検出されるように改善
...
- 🌙 月食・日食予測機能の大幅改善
- 🛡️ 無限ループ防止のためのセーフガード追加（最大反復回数20回）
- ⏱️ 月食の継続時間情報を表示に追加（皆既・部分・半影の継続時間を分単位で表示）
- 📝 月食に「世界中の広い範囲で観測可能」、日食に「観測可能地域は限定的です」の注記を追加

### v3.1.6 (2026-01-17)
**パッチアップデート**

- 📊 Google Analytics 4（GA4）トラッキングコードを追加し、アクセス統計機能を実装
...
- 🛰️ TLEベース of ISS追跡機能を実装（satellite.jsライブラリ使用）
...
プロジェクトリンク: [https://github.com/m-sakaimono/weather_dashboard](https://github.com/m-sakaimono/weather_dashboard)
